...
export default function Restrodetail() {
  ...
  const title = restrodetails?.cards[2].card.card.info.name;
  const styles = restrodetails?.cards[2]?.card?.card?.info?.labels[2]?.message;
  const location = `${restrodetails?.cards[2]?.card?.card?.info?.areaName} ${restrodetails?.cards[2]?.card?.card?.info?.locality} ${restrodetails?.cards[2]?.card?.card?.info?.sla?.lastMileTravelString}`;
  const ratings = restrodetails?.cards[2]?.card?.card?.info?.avgRatingString;
  const totalratings =
    restrodetails?.cards[2]?.card?.card?.info?.totalRatingsString;
  const offers =
    restrodetails?.cards[2]?.card?.card?.gridElements?.infoWithStyle?.offers;
    ...
    <div className="restrodetailcontainer">
      ...
      <div className="d-flex gap-3 align-center px-2">
        ...
        <h5 style={itemdetailstyle}>
          {restrodetails?.cards[2]?.card?.card?.info?.sla?.slaString}
        </h5>
        <h5 style={itemdetailstyle}>
          {restrodetails?.cards[2]?.card?.card?.info?.costForTwoMessage}{" "}
        </h5>
      </div>
      <hr className="header-separator" />
      ...
      <ItemsbyCategory
        items={
          restrodetails?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards
        }
      />
    </div>
  );
